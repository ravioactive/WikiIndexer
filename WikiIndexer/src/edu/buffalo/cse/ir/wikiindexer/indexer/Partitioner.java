...
public class Partitioner {
    ...
    private final static int MAX_PARTITION = 27;
    private final static int MIN_PARTITION = 1;
	/**
	...
	public static int getPartitionNumber (String term) {
        int partition = MIN_PARTITION;
		if(!term.isEmpty()) {
            char inital = term.charAt(0);
            if(Character.isAlphabetic(inital)) {
                partition = ((int)Character.toUpperCase(inital))-((int)'A');
            }
        }
